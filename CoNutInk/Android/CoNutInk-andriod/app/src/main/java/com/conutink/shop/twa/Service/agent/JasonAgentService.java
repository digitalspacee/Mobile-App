package com.conutink.shop.twa.Service.agent;
...
import com.conutink.shop.twa.Core.JasonParser;
import com.conutink.shop.twa.Core.JasonViewActivity;
import com.conutink.shop.twa.Helper.JasonHelper;
import com.conutink.shop.twa.Launcher.Launcher;
import com.conutink.shop.twa.R;
            ...
            if (context.agents.has(id)) {
                ...
                if (id.startsWith("$webcontainer")) {
                    ...
                    if (agent.findViewById(42) == null) {
                        ...
                        animatedImageView = new ImageView(context);
                        // animatedImageView.setImageResource(R.drawable.zing);

                        Glide.with(context)
                                .load(R.drawable.zing)
                               // .apply(RequestOptions.placeholderOf(R.drawable.zing)) // Optional placeholder image
                               // .transition(DrawableTransitionOptions.withCrossFade()) // Optional crossfade transition
                                .diskCacheStrategy(DiskCacheStrategy.ALL)
                                .into(animatedImageView);

                        // Example: Set layout parameters
                        ...
                        animatedImageView.setScaleX(0.25f);
                        animatedImageView.setScaleY(0.25f);
                        ...
                        pBar.setId(42);
                        animatedImageView.setId(42);
                    } else {
                        pBar = (ProgressBar) agent.findViewById(42);
                        animatedImageView = (ImageView) agent.findViewById(42);
                    }
                    final ProgressBar progressBar = pBar;
                    final ImageView animatedImageView1 = animatedImageView;
                    agent.addView(animatedImageView1);
                    //agent.setWebChromeClient(new WebChromeClient());
                    agent.setWebChromeClient(new WebChromeClient() {
                        public void onProgressChanged(WebView view, int progress) {
                            progressBar.setProgress(progress);
                                if (progress == 100) {
                                    animatedImageView1.setVisibility(View.GONE);

                                } else {
                                    animatedImageView1.setVisibility(View.VISIBLE);
                                }
                        }
                    });
                } else {
                ...
                agent.setWebViewClient(new WebViewClient() {
                    ...
                    @Override public boolean shouldOverrideUrlLoading(WebView view, String url) {
                        // resolve
                        System.out.println("Should Override Url Loading: "+url);
                                ...
                                if (options.has("action")) {
                                   ...
                                   JasonParser.getInstance(context).setParserListener(new JasonParser.JasonParserListener() {
                                    ...
                                    if (parsed.has("type") && parsed.getString("type").equalsIgnoreCase("$default")) {
                                        System.out.println("Parsed No False");
                                        return false;
                                    } else {
                                        System.out.println("Parsed No True");
                                        Intent intent = new Intent("call");
                ...
                agent.setScrollBarStyle(View.SCROLLBARS_INSIDE_OVERLAY);
